...
class XEyGCNLayer(nn.Module):
    ...
    def _edge_gate_logits_factorized(self, X, E, y):
        """
        Compute edge-gate logits without materializing cat([x_i, x_j, e_ij, y]).
        Mathematically equivalent to self.edge_gate_mlp(edge_in).
        """
        lin0 = self.edge_gate_mlp[0]
        W = lin0.weight
        b = lin0.bias

        dx, de = self.dx, self.de
        W_xi = W[:, :dx]
        W_xj = W[:, dx:2 * dx]
        W_e = W[:, 2 * dx:2 * dx + de]
        W_y = W[:, 2 * dx + de:]

        h_xi = F.linear(X, W_xi, None)       # [B,N,H]
        h_xj = F.linear(X, W_xj, None)       # [B,N,H]
        h_e = F.linear(E, W_e, None)         # [B,N,N,H]
        h_y = F.linear(y, W_y, b)            # [B,H]

        h = h_xi.unsqueeze(2) + h_xj.unsqueeze(1) + h_e + h_y[:, None, None, :]
        h = self.edge_gate_mlp[1](h)         # GELU
        h = self.edge_gate_mlp[2](h)         # Dropout
        logits = self.edge_gate_mlp[3](h)    # Linear(H -> 1)
        return logits

    def _edge_upd_factorized(self, X_out, E):
        """
        Compute edge update without materializing cat([x_i, x_j, e_ij]).
        Mathematically equivalent to self.edge_upd(e_in).
        """
        lin0 = self.edge_upd[0]
        W = lin0.weight
        b = lin0.bias
        ...
        h_xi = F.linear(X_out, W_xi, None)   # [B,N,H2]
        h_xj = F.linear(X_out, W_xj, None)   # [B,N,H2]
        h_e = F.linear(E, W_e, b)            # [B,N,N,H2]

        h = h_xi.unsqueeze(2) + h_xj.unsqueeze(1) + h_e

        for layer in list(self.edge_upd)[1:]:
            h = layer(h)
        return h
    ...
    def forward(self, X, E, y, node_mask):
        ...
        if self._gin_enabled:
            ...
            if self.use_edge_gating:
                logits = self._edge_gate_logits_factorized(X, E, y)
            ...
            E_upd = self._edge_upd_factorized(X_out, E)
